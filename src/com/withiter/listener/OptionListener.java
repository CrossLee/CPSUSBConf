...
import com.withiter.frame.TemperatureDialog;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (MainFrame.CURRENT_PAGE.equals("video")) {
			if (e.getSource() == jbtnAdd) {
				...
				if (!value) {
					...
					System.out.println("vNeedToDelete.toString() : "
							+ vNeedToDelete.toString());
					System.out.println("remainedList.toString(): "
							+ remainedList.toString());
		...
		if (MainFrame.CURRENT_PAGE.equals("temperature")) {
			if (e.getSource() == jbtnEdit) {
				System.out.println("Temperature edit button clicked!");
				DataTable table = MainPanel.instance().getTable();
				int rows = table.getRowCount();
				if (rows < 1) {
					JOptionPane.showMessageDialog(null, "未检测到温度信息", "提示",
							JOptionPane.OK_OPTION);
					return;
				} else {
					String tempStr = (String) table.getValueAt(0, 0);
					TemperatureDialog.instance().open(tempStr);
				}
			} else if (e.getSource() == jbtRefresh) {
				MainPanel.instance().refresh();
			} else if (e.getSource() == jbtExit) {
